...
module.exports = function(query, cb) {
    ...
    if (query.match(/^[a-zA-Z\_\-\.]+$/)){
        searches.push(function(callback){
            Modules.findOne({name: query}, function(err, doc){
                if (doc){
                    callback(null, [{exact: true, score: 10, obj: doc}]);
                }else{
                    callback(null, []);
                }
            });
    ...
    async.parallel(searches, function(err, results){
        results = results[0].concat(results[1]).filter(Boolean);
        rankResults(results, cb);
    });

    function rankResults(results, cb){
        ...
        results.sort(function(one, other){
            var oneScore = one.score * one.awesomeness * one.obj.browserifiability
            var otherScore = other.score * other.awesomeness * other.obj.browserifiability
        ...
        cb(null, results.map(function(result) {
            ...
            return {
                ...
                awesomeness: result.awesomeness.toFixed(2),
                ...
                npmDownloadsLastMonth: module.npmDownloadsLastMonth,
...
function search(query, cb) {
    ...
    Modules.db.db.command(cmd, function(err, data) {
        ...
        debug('Search returned', data.results.length, 'results', 
